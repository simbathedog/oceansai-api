...
r.get("/catalog/grades/:grade/subjects/:subject/modules", (req, res) => {
    ...
    const view = req.query.view || "outline";
    const subject = subjects.find(s => s.slug === subjectSlug);
    if (!subject)
        return res.status(404).json({ ok: false, error: "subject_not_found" });
    const mods = modules
...
function autograde(lesson, response) {
...
r.post("/me/attempts", (req, res) => {
    const u = req.user?.id || "anon";
    const { lessonId, moduleId, response } = req.body;
    ...
    if (max > 0) {
        const status = score === max ? "completed" : "in_progress";
        const now = new Date().toISOString();
...
r.get("/me/attempts", (req, res) => {
    ...
    if (lessonId)
        data = data.filter(a => a.lessonId === lessonId);
    if (moduleId)
        data = data.filter(a => a.moduleId === moduleId);
    // newest first
